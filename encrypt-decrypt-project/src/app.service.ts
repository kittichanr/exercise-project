...
import { DataDto, PayloadDto, ResponseDecryptDto, ResponseEncryptDto } from './dto';
import { createCipheriv, randomBytes, createDecipheriv, publicDecrypt, privateEncrypt, createHash } from 'crypto';
...
export class AppService {
  async getEncrypt(payload: PayloadDto): Promise<ResponseEncryptDto> {
    ...
    const privateKey = fs.readFileSync(__dirname + "/key/rsa_1024_priv.pem", { encoding: "utf8" });
  ...
  async getDecrypt(data: DataDto): Promise<ResponseDecryptDto> {
    ...
    // decrypt key
    const publicKey = fs.readFileSync(__dirname + "/key/rsa_1024_pub.pem", { encoding: "utf8" });
